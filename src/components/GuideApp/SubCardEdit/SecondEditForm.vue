      ...
      <v-card-text>
        <v-card v-if="editAudio.length > 0" width="320" class="pt-2 px-2">
          ...
          <v-dialog v-model="deleteDialogAudio" width="400px">
            <v-card class="rounded-4 pb-4">
              <v-card-title class="mb-2 text-white ps-4 fs-4" style="background-color: #BA1A1A;">Delete Audio</v-card-title>
              <v-container class="px-4 d-flex flex-column align-items-center">
                <v-icon color="#BA1A1A" size="80" class="mt-2 mdi mdi-trash-can-outline"></v-icon>
  ...
  methods: {
    ...
    async editTopic() {
      ...
      if (valid) {
        ...
        try {
          ...
          if (this.main == false) {
            response = await this.$store.dispatch('guide/updateSub2Topic', payload)
          }
          if (response) {
            this.subload = false;
    ...
    async handleImage(event) {
      ...
      try {
        ...
        if (this.main == false) {
          response = await this.$store.dispatch('guide/updateSub2Image', formData);
        }
        if (response) {
    ...
    async addImage(event) {
      ...
      try {
        ...
        if (this.main == false) {
            response = await this.$store.dispatch('guide/uploadSub2Images', payload);
        } 
        if (response) {
    ...
    async handleVideo(event) {
      ...
      try {
        ...
        if (this.main == false) {
          ...
          response = await this.$store.dispatch('guide/updateSub2Media', payload);
        }
        if (response) {
    ...
    async deleteAudio() {
      ...
      try {
        ...
        if (this.main == false) {
          response = await this.$store.dispatch('guide/deleteSub2Audio', this.editAudio[0].dtId);
        }
        if (response) {
